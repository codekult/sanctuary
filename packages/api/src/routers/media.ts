...
export const mediaRouter = router({
  list: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      const conditions = [];
      if (input.observationId) conditions.push(eq(media.observationId, input.observationId));
      if (input.individualId) conditions.push(eq(media.individualId, input.individualId));
  ...
  reorder: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      for (const item of input.items) {
        await ctx.db.update(media).set({ sortOrder: item.sortOrder }).where(eq(media.id, item.id));
      }
      return { success: true };
    }),
});
